...
export function createWorkoutExercise(exercise, order = 0) {
  return {
    ...
    pacePhases: exercise.pacePhases || null,
    order,
...
export function syncActiveWorkoutWithRoutine(workout, routine, library = []) {
  if (!workout || !routine) {
    return workout;
  }

  const resolved = resolveRoutineExercises(routine, library);
  const previous = Array.isArray(workout.exercises) ? workout.exercises : [];
  const byExerciseId = new Map();
  for (const exercise of previous) {
    if (!exercise?.exerciseId || byExerciseId.has(exercise.exerciseId)) {
      continue;
    }
  ...
  const nextExercises = resolved.map((libExercise, index) => {
    const exerciseId = libExercise.id || libExercise.exerciseId;
    const existing = exerciseId ? byExerciseId.get(exerciseId) : null;
    if (existing) {
      ...
      return {
        ...
        type:
          libExercise.type === EXERCISE_TYPE.TIMED
            ? EXERCISE_TYPE.TIMED
            : EXERCISE_TYPE.REPS,
        restSeconds: libExercise.restSeconds ?? existing.restSeconds,
        imagePath: libExercise.imagePath || existing.imagePath || null,
        notes: libExercise.notes || existing.notes || null,
        defaultSets: libExercise.defaultSets ?? existing.defaultSets,
        defaultReps: libExercise.defaultReps || existing.defaultReps || null,
        defaultWeightKg:
          libExercise.defaultWeightKg ?? existing.defaultWeightKg ?? null,
        defaultDurationSeconds:
          libExercise.defaultDurationSeconds ??
          existing.defaultDurationSeconds ??
          null,
        durationRange: libExercise.durationRange || existing.durationRange || null,
        pacePhases:
          libExercise.pacePhases !== undefined
            ? libExercise.pacePhases
            : existing.pacePhases || null,
        order: index,
      };
    }
    return createWorkoutExercise(libExercise, index);
  });

  const orphans = previous.filter((exercise) => {
    if (!exercise?.exerciseId || usedIds.has(exercise.exerciseId)) {
      return false;
    }
  ...
  const exercises = [
    ...nextExercises,
    ...orphans.map((exercise, index) => ({
      ...exercise,
      order: nextExercises.length + index,
    })),
  ];

  const currentStillPresent = exercises.some(
    (exercise) => exercise.workoutExerciseId === workout.currentExerciseId,
  );
  ...
  return {
    ...
    exercises,
    routineSnapshot: snapshotRoutine(routine, library),
    currentExerciseId: currentStillPresent ? workout.currentExerciseId : null,
  };
}
...
export function getRestRemainingMs(timer, now = Date.now()) {
  if (!timer) {
    return 0;
  }
  if (timer.pausedAt && Number(timer.remainingMs) >= 0) {
    return Math.max(0, timer.remainingMs);
  }
  return getTimerRemainingMs(timer, now);
}
...
export function adjustRestTimer(workout, deltaSeconds, now = new Date()) {
  ...
  if (timer.pausedAt) {
    const nextRemaining = Math.max(0, getRestRemainingMs(timer, now.getTime()) + deltaSeconds * 1000);
    if (nextRemaining <= 0) {
      return {
        ...workout,
        restTimer: null,
  ...
  const nextEnds = new Date(timer.endsAt).getTime() + deltaSeconds * 1000;
...
export function adjustTimedTimer(workout, deltaSeconds, now = new Date()) {
  if (!workout || !deltaSeconds) {
    return workout;
  }
  ...
  if (active?.endsAt) {
    const nextEnds = new Date(active.endsAt).getTime() + deltaSeconds * 1000;
    if (nextEnds <= nowMs) {
      return applyTimedSetComplete(
        workout,
        active.workoutExerciseId,
        active.setId,
        now,
      );
    }
  ...
  const pausedSetId = workout.pausedTimedSetId;
  const saved = pausedSetId ? workout.timedTimers?.[pausedSetId] : null;
  if (!pausedSetId || !saved) {
    return workout;
  }

  const nextRemaining = Math.max(0, (Number(saved.remainingMs) || 0) + deltaSeconds * 1000);
  if (nextRemaining <= 0) {
    return applyTimedSetComplete(
      workout,
      saved.workoutExerciseId,
      pausedSetId,
      now,
    );
  }
...
export function pauseRestTimer(workout, now = new Date()) {
  const timer = workout?.restTimer;
...
export function resumeRestTimer(workout, now = new Date()) {
  const timer = workout?.restTimer;
  if (!timer?.pausedAt) {
    return workout;
  }

  const remainingMs = getRestRemainingMs(timer, now.getTime());
  if (remainingMs <= 0) {
    return {
      ...workout,
      restTimer: null,
  ...
  return {
    ...
    restTimer: {
      ...
      endsAt: new Date(now.getTime() + remainingMs).toISOString(),
    },
  };
}

export function toggleRestPause(workout, now = new Date()) {
  if (!workout?.restTimer) {
    return workout;
  }
  return workout.restTimer.pausedAt
    ? resumeRestTimer(workout, now)
    : pauseRestTimer(workout, now);
}
...
export function resetTimedSet(workout, workoutExerciseId, setId, now = new Date()) {
  const exercise = getWorkoutExercise(workout, workoutExerciseId);
  const set = exercise?.sets?.find((item) => item.id === setId);
  if (!workout || !set || set.completed) {
    return workout;
  ...
  return {
    ...
    timedTimers: clearTimedTimerEntry(getTimedTimerMap(workout), setId),
  };
}
...
export function applyExerciseDraft(workout, workoutExerciseId, nextSets, options = {}) {
  return mapWorkoutExercise(workout, workoutExerciseId, (exercise) => {
    ...
    const weightValues = sets.map((set) => set.weightKg);
    const durationValues = sets
      .map((set) => set.durationSeconds)
      .filter((value) => value != null && Number.isFinite(Number(value)))
      .map((value) => Math.round(Number(value)));

    let defaultReps = exercise.defaultReps || null;
    if (!timed && repsValues.length > 0) {
      defaultReps = {
    ...
    let defaultWeightKg = exercise.defaultWeightKg ?? null;
    if (!timed) {
      const sameWeight = weightValues.every((value) => value === weightValues[0]);
      defaultWeightKg = sameWeight ? weightValues[0] ?? null : weightValues.find((value) => value != null) ?? null;
    }

    let defaultDurationSeconds = exercise.defaultDurationSeconds ?? null;
    let durationRange = exercise.durationRange || null;
    if (timed && durationValues.length > 0) {
      const min = Math.min(...durationValues);
      const max = Math.max(...durationValues);
      defaultDurationSeconds = max;
      durationRange = min === max ? null : { min, max };
    }

    const pacePhases = timed
      ? "pacePhases" in options
        ? normalizePacePhases(options.pacePhases)
        : exercise.pacePhases || null
      : null;
...
export function reorderWorkoutExercises(workout, visibleOrderedIds) {
  if (!workout || !Array.isArray(visibleOrderedIds) || visibleOrderedIds.length === 0) {
    return workout;
  }

  const previous = workout.exercises || [];
  ...
  const byId = new Map(
    previous.map((exercise) => [exercise.workoutExerciseId, exercise]),
  );
  ...
  let visibleIndex = 0;
  const exercises = previous.map((exercise) => {
    if (!visibleSet.has(exercise.workoutExerciseId)) {
      return exercise;
    }
    const next = byId.get(visibleOrderedIds[visibleIndex]);
    visibleIndex += 1;
    return next;
  });

  const unchanged = exercises.every(
    (exercise, index) =>
      exercise.workoutExerciseId === previous[index].workoutExerciseId,
  );
  if (unchanged) {
    return workout;
  }
...
export function removeExerciseFromWorkout(workout, workoutExerciseId) {
  if (!workout || !workoutExerciseId) {
    return workout;
  }
  ...
  const exercises = previous
    .filter((exercise) => exercise.workoutExerciseId !== workoutExerciseId)
    .map((exercise, index) => ({ ...exercise, order: index }));

  if (exercises.length === previous.length) {
    return workout;
  }

  let timedTimer = workout.timedTimer;
  if (timedTimer?.workoutExerciseId === workoutExerciseId) {
    timedTimer = null;
  }

  const timedTimers = { ...(workout.timedTimers || {}) };
  for (const set of previous.find((item) => item.workoutExerciseId === workoutExerciseId)?.sets || []) {
    delete timedTimers[set.id];
  }
  ...
  return {
    ...
    exercises,
    currentExerciseId:
      workout.currentExerciseId === workoutExerciseId
        ? exercises[0]?.workoutExerciseId || null
        : workout.currentExerciseId,
    timedTimer,
    timedTimers,
    pausedTimedSetId:
      previous
        .find((item) => item.workoutExerciseId === workoutExerciseId)
        ?.sets?.some((set) => set.id === workout.pausedTimedSetId)
        ? null
        : workout.pausedTimedSetId,
  };
}
...
export function getSessionActivityFlags(session) {
  const all = getSessionExercises(session);
  const performed = all.filter(exerciseWasPerformed);
  const source = performed.length > 0 ? performed : all;
  return {
    hasStrength: source.some(isStrengthExercise),
    hasCardio: source.some(isCardioExercise),
  };
}
