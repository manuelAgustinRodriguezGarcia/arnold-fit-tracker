...
export function createExerciseRecord(input = {}) {
  ...
  return {
    ...
    pacePhases: type === EXERCISE_TYPE.TIMED ? normalizePacePhases(input.pacePhases) : null,
...
export function normalizePacePhases(input) {
  if (!input || typeof input !== "object") {
    return null;
  }
  const startSeconds = Math.round(Number(input.startSeconds));
  const midSeconds = Math.round(Number(input.midSeconds));
  if (!Number.isFinite(startSeconds) || startSeconds < 1) {
    return null;
  }
  if (!Number.isFinite(midSeconds) || midSeconds < 1) {
    return null;
  }
  return {
    startSeconds,
...
export function getTimedPacePhase(elapsedMs, durationMs, pacePhases) {
  const pace = normalizePacePhases(pacePhases);
  if (!pace || !Number.isFinite(elapsedMs) || !Number.isFinite(durationMs) || durationMs <= 0) {
    return null;
  }
  const elapsed = Math.max(0, elapsedMs);
  const startMs = pace.startSeconds * 1000;
  if (elapsed < startMs) {
    return "start";
  }
  const midStart = durationMs / 2;
  const midEnd = midStart + pace.midSeconds * 1000;
  if (elapsed >= midStart && elapsed < midEnd) {
    return "mid";
  }
  return null;
}
