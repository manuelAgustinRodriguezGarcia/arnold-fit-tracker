...
export function createExerciseRecord(input = {}) {
  ...
  const restSeconds = Math.max(0, Number(input.restSeconds) || DEFAULT_REST_SECONDS);
...
export function formatExerciseSummary(exercise) {
  ...
  if (exercise.type === EXERCISE_TYPE.TIMED) {
    return `${exercise.defaultSets} series · ${formatDurationRange(exercise)} · ${exercise.restSeconds} s descanso`;
  }
  const weight =
    exercise.defaultWeightKg != null ? ` · ${exercise.defaultWeightKg} KG` : "";
  return `${exercise.defaultSets} series · ${formatReps(exercise.defaultReps)} reps · ${exercise.restSeconds} s${weight}`;
}
