...
import { getCompletedSets, getExerciseVolume } from "./workoutSets";
...
export function getLongestSessionDuration(sessions, excludeId = null) {
...
export function isLongestDurationSession(session, sessions) {
  if (!session) {
    return false;
  }
  const duration = Number(session.durationSeconds) || 0;
  ...
  const previousMax = getLongestSessionDuration(sessions, session.id);
  return duration > previousMax;
}
...
export function getActivityByDay(sessions, range) {
  ...
  while (cursor <= range.end) {
    ...
    for (const session of daySessions) {
      const name = session.routineName || session.routineSnapshot?.name || "Entrenamiento";
      if (!routineNames.includes(name)) {
