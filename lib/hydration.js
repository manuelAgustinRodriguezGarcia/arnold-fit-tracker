...
export function normalizeBottleCapacityMl(value) {
  if (value == null || value === "") {
    return 0;
  }
  ...
  return Math.min(MAX_WATER_ML, Math.round(raw));
}

export function maxBottlesForCapacity(capacityMl) {
  const capacity = normalizeBottleCapacityMl(capacityMl);
  if (capacity <= 0) {
    return 0;
  }
  return Math.floor(MAX_WATER_ML / capacity);
}

export function waterMlFromBottles(count, capacityMl) {
  const capacity = normalizeBottleCapacityMl(capacityMl);
  const bottles = Math.max(0, Math.round(Number(count) || 0));
  if (capacity <= 0 || bottles <= 0) {
    return 0;
  }
  return Math.min(MAX_WATER_ML, bottles * capacity);
}
