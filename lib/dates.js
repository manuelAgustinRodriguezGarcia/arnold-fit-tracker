...
export function getMonthNames(style = "long") {
  return Array.from({ length: 12 }, (_, month) => {
    const label = new Date(2020, month, 1).toLocaleDateString(DATE_LOCALE, {
      month: style,
    });
...
export function formatMonthYear(year, monthIndex) {
  const label = new Date(year, monthIndex, 1).toLocaleDateString(DATE_LOCALE, {
    month: "long",
    year: "numeric",
  });
  return label.charAt(0).toUpperCase() + label.slice(1);
...
export function getMonthCells(year, monthIndex) {
  ...
  for (let index = 0; index < 42; index += 1) {
    cells.push({
      ...
      day: cursor.getDate(),
      outside: cursor.getMonth() !== monthIndex,
    });
...
export function getCalendarYearRange(sessions = [], now = new Date()) {
  const current = toDate(now).getFullYear();
  let min = current;
  let max = current;
  for (const session of sessions) {
    const year = toDate(session.endedAt || session.startedAt).getFullYear();
