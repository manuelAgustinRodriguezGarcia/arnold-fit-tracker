...
export const DEFAULT_STATE = {
  ...
  sessions: [],
  activeWorkout: null,
  settings: { themePalette: "classic", appearance: "light", bottleCapacityMl: 0 },
  migrationsApplied: [],
};
  ...
  try {
    ...
    try {
      ...
      const settingsChanged =
        !parsed.settings ||
        parsed.settings.themePalette !== migrated.settings.themePalette ||
        parsed.settings.appearance !== migrated.settings.appearance;
      if (migrated.version !== parsed.version || appliedChanged || settingsChanged) {
        writeState(migrated);
      }
      return migrated;
