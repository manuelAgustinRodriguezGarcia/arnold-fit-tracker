...
import { normalizeBottleCapacityMl } from "./hydration";
...
export const MIGRATION_STRETCH_PRESETS_V2 = "stretch-presets-v2";
...
export function normalizeStateShape(state) {
  return {
    ...
    settings: {
      ...
      bottleCapacityMl: normalizeBottleCapacityMl(state?.settings?.bottleCapacityMl),
    },
...
export function applyNamedMigrations(state) {
  ...
  if (!applied.has(MIGRATION_STRETCH_PRESETS_V2)) {
    current = migrateStretchPresets(current);
    applied.add(MIGRATION_STRETCH_PRESETS_V2);
  }
...
export function needsNamedMigration(state) {
  ...
  return (
    ...
    !applied.includes(MIGRATION_STRETCH_PRESETS_V2)
  );
}
